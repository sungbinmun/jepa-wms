...
def make_log_dir(
    ...
    group = group or ""
    root = pathlib.Path(log_dir) / group

    # Prefer deterministic naming based on config filename with numeric suffix (_1, _2, ...)
    if config_name is not None:
        base_name = pathlib.Path(config_name).stem
        count = 1
        while True:
            dir_name = f"{base_name}_{count}"
            log_path = root / dir_name
            if not log_path.is_dir():
                log_path.mkdir(parents=True, exist_ok=True)
                return log_path
            count += 1

    # Fallback: timestamp-based naming (previous behaviour)
