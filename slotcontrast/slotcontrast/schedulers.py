...
def build(
    ...
    if name == "constant":
        ...
        scheduler_fn = functools.partial(
            ...
            warmup_init_step=config.get("warmup_init_step", 0),
        )
    elif name == "exp_decay_with_warmup":
        scheduler_fn = functools.partial(
...
def linear_warmup(step: int, warmup_steps: int, warmup_init_step: int = 0) -> float:
    """Linear warmup.

    Function maps current step or epoch to factor of learning rate schedules.
    """
    step = max(0, step - warmup_init_step)
    if warmup_steps > 0:
