...
def build(config, name: str):
    if name == "SpatialBroadcastDecoder":
        ...
        return SpatialBroadcastDecoder(
            ...
            **config_as_kwargs(config, ("backbone", "output_transform", "pos_embed")),
        )
...
class MLPDecoder(nn.Module):
    ...
    def __init__(
        ...
        frozen: bool = False,
        pos_embed_mode: str = "add",
    ):
        super().__init__()
        self.outp_dim = outp_dim
        self.n_patches = n_patches
        self.eval_output_size = list(eval_output_size) if eval_output_size else None
        if pos_embed_mode not in ("add", "concat"):
            raise ValueError("`pos_embed_mode` should be 'add' or 'concat'")
        self.pos_embed_mode = pos_embed_mode

        mlp_in = inp_dim if pos_embed_mode == "add" else inp_dim * 2
        self.mlp = networks.MLP(
            mlp_in, outp_dim + 1, hidden_dims, activation=activation, frozen=frozen
        )
    ...
    def forward(self, slots: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.pos_embed_mode == "add":
            ...
            slots = torch.cat((slots, pos_emb), dim=-1)
        ...
        return {
            ...
            # Keep per-slot reconstructions so we can visualize slot-wise feature maps.
            "slot_reconstructions": recons,
        }
...
class SlotMixerDecoder(nn.Module):
    ...
    def forward(self, slots: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        #attn[0,..., 0].reshape(336, 336).unsqueeze(0).unsqueeze(1)
