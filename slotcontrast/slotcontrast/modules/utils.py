...
class Permute(nn.Module):
    """Permute tensor dimensions using torch.permute."""
...
def _merge_pos(x: torch.Tensor, pos: torch.Tensor, mode: str, dim: int) -> torch.Tensor:
    if mode == "add":
        return x + pos
    if mode == "concat":
        return torch.cat((x, pos), dim=dim)
    raise ValueError(f"Unknown pos embed mode {mode}. Use 'add' or 'concat'.")


class CoordinatePositionEmbed(nn.Module):
    """Coordinate positional embedding as in Slot Attention."""

    def __init__(self, dim: int, size: Tuple[int, int], mode: str = "add"):
        super().__init__()
        if isinstance(size, int):
            size = (size, size)
        if mode not in ("add", "concat"):
            raise ValueError("`mode` should be 'add' or 'concat'")
        self.mode = mode
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pos = self.proj(self.grid)
        if pos.ndim == 3:
            pos = pos.unsqueeze(0)
        pos = pos.expand(x.shape[0], -1, -1, -1)
        if self.mode == "add" and x.shape[1] != pos.shape[1]:
            raise ValueError(f"Add mode expects {pos.shape[1]} channels, got {x.shape[1]}")
        return _merge_pos(x, pos, self.mode, dim=1)


class TokenCoordinatePositionEmbed(nn.Module):
    """Coordinate positional embedding for (B, N, C) token inputs."""
    ...
    def __init__(self, dim: int, size: Tuple[int, int], mode: str = "add"):
        ...
        self.pe = CoordinatePositionEmbed(dim, self.size, mode=mode)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        assert x.ndim == 3, f"Expected input shape (batch, patches, channels), but got {x.shape}"
        b, n, c = x.shape
        h, w = self.size
        if n != h * w:
        ...
        x_map = self.pe(x_map)
        c2 = x_map.shape[1]
        return x_map.reshape(b, c2, n).transpose(1, 2)

class TokenCoordCat(nn.Module):
    """Concat 4D coord PE to tokens, then project back to feature dim.

    Input:  x (B, N, C) where N == H*W and C == dim
    Output: (B, N, out_dim)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Precompute 4D coordinate grid (N, 4) in [-1, 1]
        self.register_buffer("grid", self._grid(self.size), persistent=False)

        # Optional learnable scaling for PE
        self.pe_scale = nn.Parameter(torch.tensor(1.0)) if learnable_pe_scale else None
        ...
        if hidden_dim is not None and hidden_dim > 0:
            ...
            layers.append(nn.Linear(int(hidden_dim), self.out_dim))
            self.proj = nn.Sequential(*layers)

            # No separate dropout needed; handled inside Sequential
            self.post_dropout = None
            self.norm = None
        else:
            # Linear-only path: optional LN before projection + optional post dropout
            self.norm = nn.LayerNorm(in_dim) if use_layernorm else None
            self.proj = nn.Linear(in_dim, self.out_dim)
            self.post_dropout = nn.Dropout(dropout) if dropout > 0.0 else None

    @staticmethod
    def _grid(size: Tuple[int, int]) -> torch.Tensor:
        """Return (H*W, 4) grid in [-1, 1] with (x, y, 1-x, 1-y)."""
        h, w = int(size[0]), int(size[1])
        y = torch.linspace(0.0, 1.0, steps=h)
        x = torch.linspace(0.0, 1.0, steps=w)
        yy, xx = torch.meshgrid(y, x, indexing="ij")  # (H, W)

        pe = torch.stack((xx, yy, 1.0 - xx, 1.0 - yy), dim=-1)  # (H, W, 4)
        pe = pe * 2.0 - 1.0  # -> [-1, 1]
        return pe.reshape(h * w, 4)  # (N, 4)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 3:
            raise ValueError(f"Expected input shape (batch, patches, channels), got {x.shape}")
        b, n, c = x.shape
        h, w = self.size
        if n != h * w:
            raise ValueError(f"Token count {n} does not match grid size {h}x{w}")
        ...
        pe = self.grid.to(device=x.device, dtype=x.dtype).unsqueeze(0).expand(b, -1, -1)  # (B, N, 4)

        if self.pe_scale is not None:
            pe = pe * self.pe_scale.to(dtype=pe.dtype)  # avoid fp32 upcast under AMP

        x_cat = torch.cat((x, pe), dim=-1)  # (B, N, C+4)

        if self.norm is not None:
            x_cat = self.norm(x_cat)

        out = self.proj(x_cat)  # (B, N, out_dim)
...
class LearnedPositionEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.expected_dims == 3:
            ...
            pos = self.pos_emb.expand(x.shape[0], -1, -1, -1)
            if self.mode == "add":
                x = x + pos
            else:
                x = torch.cat((x, pos), dim=1)
