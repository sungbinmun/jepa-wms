...
def create_grid_frame(frames, grid_size=(2, 4), image_size=(224, 224), padding=2):
...
def create_grid_frame_rgb(frames, grid_size=(2, 8), image_size=(224, 224), padding=2):
    """
...
def mix_inputs_with_masks(inputs, outputs, softmasks=False):
    ...
    # resize masks to match the visualization frames' spatial size
    video = inputs["video_visualization"]
    frame_size = tuple(video.shape[-2:])
    num_grid_frames = masks_video.shape[1] + 1  # +1 for the original frame slot
    grid_cols = min(num_grid_frames, 8)
    grid_rows = int(np.ceil(num_grid_frames / grid_cols))
    grid_size = (grid_rows, grid_cols)
    resizer = Resize(frame_size, mode="bilinear")
    masks_video = resizer(masks_video).detach().cpu()
    ...
    for t in range(masks_video.shape[0]):  # Iterate through each time step
        frames = [
            masks_video[t, i].numpy() for i in range(masks_video.shape[1])
        ...
        grid_frame = create_grid_frame(frames, grid_size=grid_size, image_size=frame_size)
        # Optional: Convert grid_frame to RGB if needed
        grid_frame_rgb = np.repeat(grid_frame[:, :, np.newaxis], 3, axis=2)
        video_frames = [
            video_cpu[0, t].permute(1, 2, 0).numpy() for _ in range(masks_video.shape[1] + 1)
        ]
        grid_video = create_grid_frame_rgb(video_frames, grid_size=grid_size, image_size=frame_size)
