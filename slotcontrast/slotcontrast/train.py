...
LOG_ENV_KEYS = ("VIDEOSAUR_LOG_PATH", "VDINOSAUR_LOG_PATH")
...
parser.add_argument("--wandb-entity", help="WandB entity name")
parser.add_argument("--wandb-group", help="WandB group name")
parser.add_argument("--wandb-name", help="WandB run name")
parser.add_argument("--wandb-id", help="WandB run id to resume")
parser.add_argument("--wandb-resume", action="store_true", help="Resume WandB run if possible")
parser.add_argument("--visnow", action="store_true", help="Force a visualization at step 0")
...
parser.add_argument(
    "--weights-only-from",
    type=pathlib.Path,
    help="Load model weights from this checkpoint or log directory (no optimizer state).",
)
...
def _set_torch_load_weights_only_false():
    # Allow full checkpoint loading for trusted runs (PyTorch 2.6 defaults to weights_only=True).
...
def _setup_callbacks(args, config, log_path: pathlib.Path, dataset=None) -> Dict[str, pl.Callback]:
    ...
    if not args.dry:
        ...
        #)
        checkpointer = pl.callbacks.ModelCheckpoint(
            dirpath=log_path / CHECKPOINT_SUBDIR,
            filename="{step}",
            monitor=None,                # ← 경고와 동일
            mode="min",                  # ← 경고와 동일
            every_n_train_steps=getattr(config, "checkpoint_every_n_steps", 10000),  # ← 동일
            every_n_epochs=0,            # ← 동일
            train_time_interval=None,    # ← 동일
            save_top_k=-1,
            verbose=args.verbose,
        )
...
def _find_latest_wandb_run_id(log_path: pathlib.Path) -> Optional[str]:
    wandb_dir = log_path / "wandb"
    ...
    if latest.exists():
        try:
            ...
            pass
    run_dirs = [p for p in wandb_dir.glob("run-*") if p.is_dir()]
    if not run_dirs:
        return None
...
def _setup_loggers(
    args, log_path: pathlib.Path, config=None
    ...
    if getattr(args, "use_wandb", False):
        wandb_project = (
            args.wandb_project or getattr(config, "experiment_group", None) or "slotcontrast"
        )
        wandb_logger_kwargs = {
            "project": wandb_project,
            "entity": args.wandb_entity,
            "group": args.wandb_group or getattr(config, "experiment_group", None),
            "name": args.wandb_name or getattr(config, "experiment_name", None),
            "save_dir": log_path,
        }
        wandb_id = args.wandb_id
        if wandb_id is None and (args.wandb_resume or args.continue_from):
            wandb_id = _find_latest_wandb_run_id(log_path)
        if wandb_id is not None:
            wandb_logger_kwargs["id"] = wandb_id
            wandb_logger_kwargs["resume"] = "allow"
        # Remove keys left unspecified so WandB can apply its own defaults.
        wandb_logger_kwargs = {k: v for k, v in wandb_logger_kwargs.items() if v is not None}
        loggers["wandb"] = pl.loggers.WandbLogger(**wandb_logger_kwargs)
...
def _resolve_checkpoint_path(path: pathlib.Path) -> pathlib.Path:
    if path.is_file() and path.suffix == ".ckpt":
        return path
    if path.is_dir():
        ckpt = utils.find_last_checkpoint(path)
        if ckpt is None:
            raise ValueError(f"No checkpoint found in {path}")
        return ckpt
    raise ValueError("Checkpoint path should be a .ckpt file or a directory")


def main(args, config_overrides=None):
    _set_torch_load_weights_only_false()
    ...
    if getattr(config, "experiment_name", None) in (None, "") and config_path.name == "train_config.yaml":
        config.experiment_name = config_path.parent.name
    if args.visnow:
        if getattr(config, "model", None) is None:
            raise ValueError("--visnow requires a model configuration")
        config.model.visualize_now = True
    ...
    if not args.dry:
        use_config_dir = (
            config_path.name == "train_config.yaml"
            and config_path.parent.is_dir()
            and args.log_dir == "./logs"
            and not args.no_sub_logdirs
        )
        env_log_path = next((os.environ[k] for k in LOG_ENV_KEYS if k in os.environ), None)
        if env_log_path:
            ...
            log_path = config_path.parent
            log_path.mkdir(parents=True, exist_ok=True)
        else:
            config_path = pathlib.Path(args.config)
            if config_path.name == "train_config.yaml":
                log_path = config_path.parent
                log_path.mkdir(parents=True, exist_ok=True)
            elif args.no_sub_logdirs:
    ...
    weights_only_path: Optional[pathlib.Path] = None
    if args.continue_from and args.weights_only_from:
        raise ValueError("--continue and --weights-only-from are mutually exclusive")
    if args.weights_only_from:
        weights_only_path = _resolve_checkpoint_path(args.weights_only_from)
    elif args.continue_from:
        ckpt_path = _resolve_checkpoint_path(args.continue_from)
    elif not args.dry:
    ...
    if weights_only_path is not None:
        log_info(f"Loading model weights from checkpoint {weights_only_path}")
        state = torch.load(weights_only_path, map_location="cpu")
        if isinstance(state, dict) and "state_dict" in state:
            state = state["state_dict"]
        missing, unexpected = model.load_state_dict(state, strict=False)
        if missing or unexpected:
            log_info(
                f"Weights-only load: missing={len(missing)} unexpected={len(unexpected)}"
            )
            if missing:
                log_info(f"Missing keys: {missing}")
            if unexpected:
                log_info(f"Unexpected keys: {unexpected}")
    ...
    elif weights_only_path is not None:
        log_info("Starting training from scratch (weights-only init)")
    else:
